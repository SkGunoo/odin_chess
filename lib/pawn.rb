...
class Pawn < ChessPiece
  ...
  def get_movable_positions(board)
    ...
    if blocked_by_another_piece?
      movable_positions + [@current_location] + catchable_chesspieces
    else
      forward_locations(board) + catchable_chesspieces
    end
  ...
  def forward_locations(board)
    ...
    if @number_of_moves == 0 && board.board_with_object[second_tile[0]][second_tile[1]].nil?
      [location, [location[0] + switch_direction_depends_on_player(1), @current_location[1]],
       [location[0] + switch_direction_depends_on_player(2), location[1]]]
